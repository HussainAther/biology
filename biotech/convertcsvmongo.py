...
with gzip.open(FILE_NAME, "rt", newline="") as f_unzip:
    ...
    for row in rows:
        if row[0] in at_d:
            ...
            c7 = int(row[7])
            left = c7 if c7<int(left_val) else left_val22c8 = int(row[8])
            right = c8 if c8>int(right_val) else right_val
            at_d[row[0]] = (int(chromosome), left, right)
        else:
            at_d[row[0]] = (int(row[5]), int(row[7]),27int(row[8]))
# Make a list with dictionaries to be stored as documents in
# MongoDB30 
markers = []
for marker in at_d:
    markers.append({'marker_id': marker, 'chromosome':at_d[marker][0], 'start': at_d[marker][1], 'end': at_d[marker][2]}) client = MongoClient(CONNECTION_STRING) db = client.TAIRDB 
